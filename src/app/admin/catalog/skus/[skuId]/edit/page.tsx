...
export default async function EditSkuPage({ params }: { params: Promise<{ skuId: string }> }) {
  ...
  const [sku, productOptions] = await Promise.all([findAdminSkuById(skuId), listProductGrantOptions()]);
  if (!sku) notFound();
  return <main className="admin-page"><header className="account-header"><Link className="wordmark" href="/">XCRUIZT<span>®</span></Link><AccountControls resolution={{ account, status: "ready" }} /></header><section className="admin-shell admin-editor"><p className="section-kicker">ADMIN / CATALOG / SKUS</p><h1>Edit {sku.name}</h1><p>การเปลี่ยนราคา สถานะ หรือ grants จะถูกบันทึกใน Audit Log</p><SkuForm action={updateSkuAction.bind(null, sku.id)} products={productOptions} sku={sku} /></section></main>;
}
