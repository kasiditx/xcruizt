...
export default async function NewVersionPage() {
  ...
  return <main className="admin-page"><header className="account-header"><Link className="wordmark" href="/">XCRUIZT<span>®</span></Link><AccountControls resolution={{ account, status: "ready" }} /></header><section className="admin-shell admin-editor"><p className="section-kicker">ADMIN / CATALOG / VERSIONS</p><h1>New Version</h1><p>Version ใหม่เริ่มเป็น Draft เสมอเพื่อป้องกัน Library ชี้ไปยังไฟล์ที่ยังไม่พร้อม</p><VersionForm action={createVersionAction} products={products} /></section></main>;
}
