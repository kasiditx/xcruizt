...
import {
  finalizeStagingSkuPackageFile,
  findStagingSkuPackageFile,
...
export async function POST(request: Request) {
  ...
  const file = await findStagingSkuPackageFile(parsed.data.fileId);
  ...
  try {
    ...
    logServerError("admin.sku_package_upload_verification_failed", {
      fileId: file.id,
      requestId,
    }, error);
  ...
  const result = await finalizeStagingSkuPackageFile({
